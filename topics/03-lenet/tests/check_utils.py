"""
Check utilities for the Module 03 assignment notebook (see assignment.ipynb).

Same harness as Modules 01-02 (the tools are module-agnostic): a relative-error
metric, centered finite-difference gradient checks, and a helper that compares a
student's result to the canonical answer key (python/lenet.py + nanograd).

In Module 03 the gradient check earns its keep on the new conv machinery: the
student writes the conv forward accumulation, the dK backward, and max-pool's
gradient routing, and these numerically confirm them. A correct analytic
gradient matches the finite-difference one to a `rel_error` around 1e-7 or
smaller.

Imported by adding tests/ to sys.path, the same pattern tests/test_agreement.py uses:

    import sys, os
    sys.path.insert(0, os.path.join("topics", "03-lenet", "tests"))
...
def compare_to_canonical(student, canonical, labels=None, tol=1e-9):
    ...
    - student, canonical: equal-length sequences of floats. `canonical` comes
      from the module's python/lenet.py (the answer key).
